export const path = {
	path_1:
		"M200-160v-80h560v80H200Zm0-140-51-321q-2 0-4.5.5t-4.5.5q-25 0-42.5-17.5T80-680q0-25 17.5-42.5T140-740q25 0 42.5 17.5T200-680q0 7-1.5 13t-3.5 11l125 56 125-171q-11-8-18-21t-7-28q0-25 17.5-42.5T480-880q25 0 42.5 17.5T540-820q0 15-7 28t-18 21l125 171 125-56q-2-5-3.5-11t-1.5-13q0-25 17.5-42.5T820-740q25 0 42.5 17.5T880-680q0 25-17.5 42.5T820-620q-2 0-4.5-.5t-4.5-.5l-51 321H200Zm68-80h424l26-167-105 46-133-183-133 183-105-46 26 167Zm212 0Z",
	path_2:
		"m305-704 112-145q12-16 28.5-23.5T480-880q18 0 34.5 7.5T543-849l112 145 170 57q26 8 41 29.5t15 47.5q0 12-3.5 24T866-523L756-367l4 164q1 35-23 59t-56 24q-2 0-22-3l-179-50-179 50q-5 2-11 2.5t-11 .5q-32 0-56-24t-23-59l4-165L95-523q-8-11-11.5-23T80-570q0-25 14.5-46.5T135-647l170-57Zm49 69-194 64 124 179-4 191 200-55 200 56-4-192 124-177-194-66-126-165-126 165Zm126 135Z",
	path_3: "m560-240-56-58 142-142H160v-80h486L504-662l56-58 240 240-240 240Z",
	color_white: "#D9D9D9",
	color_black: "#000",
	color_primary: "#53eafd",
};

export const intro = {
	title: "services",
	butt_title: "premium services",
	description:
		"Five decades of expertise in creating bespoke furniture solutions for Dubai's most discerning clients",
};

export const cards = [
	{
		banner: "bg-[url(/src/assets/card/card-banner/1.jpeg)]",
		icon: {
			shape:
				"M400-120q-17 0-28.5-11.5T360-160v-480H160q0-83 58.5-141.5T360-840h240v120l120-120h80v320h-80L600-640v480q0 17-11.5 28.5T560-120H400Zm40-80h80v-240h-80v240Zm0-320h80v-240H360q-26 0-49 10.5T271-720h169v200Zm40 40Z",
			color: "#fff",
		},

		descrip: {
			title: "Custom Furniture Manufacturing",
			description:
				"Bespoke pieces crafted to your exact specifications using premium materials and traditional techniques.",
		},

		list: ["100% Custom Design", "Premium Materials", "+2 more"],
	},

	{
		banner: "bg-[url(/src/assets/card/card-banner/2.jpeg)]",
		icon: {
			shape:
				"M760-120 480-400l-94 94q8 15 11 32t3 34q0 66-47 113T240-80q-66 0-113-47T80-240q0-66 47-113t113-47q17 0 34 3t32 11l94-94-94-94q-15 8-32 11t-34 3q-66 0-113-47T80-720q0-66 47-113t113-47q66 0 113 47t47 113q0 17-3 34t-11 32l494 494v40H760ZM600-520l-80-80 240-240h120v40L600-520ZM240-640q33 0 56.5-23.5T320-720q0-33-23.5-56.5T240-800q-33 0-56.5 23.5T160-720q0 33 23.5 56.5T240-640Zm240 180q8 0 14-6t6-14q0-8-6-14t-14-6q-8 0-14 6t-6 14q0 8 6 14t14 6ZM240-160q33 0 56.5-23.5T320-240q0-33-23.5-56.5T240-320q-33 0-56.5 23.5T160-240q0 33 23.5 56.5T240-160Z",
			color: "#fff",
		},

		descrip: {
			title: "Refurbishment & Re‑upholstery",
			description:
				"Expert reupholstery and custom cushioning with the finest fabrics and leather from European suppliers.",
		},

		list: ["European Fabrics", "Custom Cushioning", "+2 more"],
	},
	{
		banner: "bg-[url(/src/assets/card/card-banner/3.jpeg)]",
		icon: {
			shape:
				"M80-120v-80h80v-640h640v640h80v80H80Zm160-560h320v-80H240v80Zm0 160h320v-80H240v80Zm0 320h480v-240h-80v73q14 10 22 25t8 32q0 29-20.5 49.5T600-240q-29 0-49.5-20.5T530-310q0-17 8-32t22-25v-73H240v240Zm400-480h80v-80h-80v80Zm0 160h80v-80h-80v80Z",
			color: "#fff",
		},

		descrip: {
			title: "Curtains & Blinds",
			description:
				"Custom window treatments that perfectly complement your interior design vision with motorized options.",
		},

		list: ["Smart Home Integration", "Custom Measurements", "+2 more"],
	},
	{
		banner: "bg-[url(/src/assets/card/card-banner/4.jpeg)]",
		icon: {
			shape:
				"M440-80q-33 0-56.5-23.5T360-160v-160H240q-33 0-56.5-23.5T160-400v-280q0-66 47-113t113-47h480v440q0 33-23.5 56.5T720-320H600v160q0 33-23.5 56.5T520-80h-80ZM240-560h480v-200h-40v160h-80v-160h-40v80h-80v-80H320q-33 0-56.5 23.5T240-680v120Zm0 160h480v-80H240v80Zm0 0v-80 80Z",
			color: "#fff",
		},

		descrip: {
			title: "Outdoor Furniture Upholstery & Covers",
			description:
				"Weather-resistant solutions for outdoor furniture with UV protection and premium outdoor fabrics.",
		},

		list: ["UV Protection", "Weather Resistant", "+2 more"],
	},
];

export const container = {
	hidden: { opacity: 0 },
	show: {
		opacity: 1,
		transition: {
			type: "spring",
			staggerChildren: 0.5, // Delay between each child animation
		},
	},
};

export const item = {
	hidden: { y: 20, opacity: 0 },
	show: { y: 0, opacity: 1 },
};
